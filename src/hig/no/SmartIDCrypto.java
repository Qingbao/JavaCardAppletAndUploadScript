...
/**
 * The generic crypto routines (the perfect world mode where all
 * the needed ciphers are supported. Current JCOP41 cards support all the
 * ciphers here.
 * 
 */
public class SmartIDCrypto {
    ...
    private boolean[] eacChangeKeys;

    MessageDigest shaDigest;
    Cipher rsaCiph;


    SmartIDCrypto(KeyStore keyStore) {
    ...
    short encryptInit(byte padding, byte[] ptext, short ptext_offset,
        ...
        if (padding == PAD_INPUT) {
            newlen = SmartIDUtil.pad(ptext, ptext_offset, ptext_len);
        }
    ...
    short unwrapCommandAPDU(byte[] ssc, APDU apdu) {
        ...
        if (buf[apdu_p] == (byte) 0x87) {
            ...
            if (do87LenBytes > 2) { // sanity check
                ISOException.throwIt(SmartIDApplet.SW_SM_DO_INCORRECT);
            }
        ...
        if (buf[apdu_p] != (byte) 0x8e) {
            ISOException.throwIt(SmartIDApplet.SW_SM_DO_MISSING);
        }
        if (buf[++apdu_p] != 8) {
            ISOException.throwIt(SmartIDApplet.SW_SM_DO_INCORRECT);
        }
        ...
        if (do87DataLen != 0) {
            ...
            plaintextLc = SmartIDUtil.calcLcFromPaddedData(buf,
    ...
    short wrapResponseAPDU(byte[] ssc, APDU aapdu,
        ...
        short do87DataLen = SmartIDUtil.lengthWithPadding(plaintextLen);
        ...
        // Reset the secure messaging keys after successful EAC Chip Authentication
        if (eacChangeKeys[0]) {
    ...
    void deriveKey(byte[] buffer, short keySeed_offset,
        ...
        // parity bits
        for (short i = key_offset; i < (short) (key_offset + SmartIDApplet.KEY_LENGTH); i++) {
            if (SmartIDUtil.evenBits(buffer[i]) == 0)
    ...
    /**
     * Chip authentication part of EAC.
     * 
     * @param pubData
