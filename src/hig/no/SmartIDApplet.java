...
/**
 * SmartID Applet according to ICAO
 * 
 * 
 */
public class SmartIDApplet extends Applet implements ISO7816 {
    ...
    public SmartIDApplet() {
        ...
        crypto = new SmartIDCrypto(keyStore);
    ...
    private short processMSE(APDU apdu, boolean protectedApdu) {
        ...
        if ((!hasEACKey() && p1 == P1_SETFORCOMPUTATION) || ((!hasEAPCertificate() || !hasSicId())
    ...
    private void processPutData(APDU apdu) {
        ...
        if (p1 == 0 && p2 == PRIVMODULUS_TAG) {
            ...
            crypto.deriveKey(buffer, buffer_p, lc, SmartIDCrypto.MAC_MODE,
                    macKey_p);
            crypto.deriveKey(buffer, buffer_p, lc, SmartIDCrypto.ENC_MODE,
                    encKey_p);
    ...
    private short processGetChallenge(APDU apdu, boolean protectedApdu, short le) {
        ...
        // For the BAC challenge the length should be 8, for the EAC challenge
       //FIXME we guess other lengths are fine too?
    ...
    private short processReadBinary(APDU apdu, short le, boolean protectedApdu) {
        ...
        len = SmartIDUtil.min((short) (buffer.length - 37),
                (short) (fileSize - offset));
        // FIXME: 37 magic
        len = SmartIDUtil.min(len, (short) buffer.length);
        len = SmartIDUtil.min(le, len);
